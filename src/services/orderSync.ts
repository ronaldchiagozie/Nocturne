...
function cartToLineItems(items: CartItem[]): OrderLineItem[] {
  return items.map((item) => {
    const line: OrderLineItem = {
      ...
      priceNgn: Math.round(item.priceNgn),
...
function buildStoreOrder(input: CheckoutSubmitInput, id: string): StoreOrder {
  return {
    ...
    subtotalNgn: Math.round(input.subtotalNgn),
    shippingNgn: Math.round(input.shippingNgn),
    totalNgn: Math.round(input.totalNgn),
    totalUsd: Math.round(input.totalUsd),
