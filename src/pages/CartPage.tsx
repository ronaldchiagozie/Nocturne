...
export function CartPage() {
  ...
  return (
    <div className="bg-cream text-canvas min-h-screen pb-[calc(5.5rem+env(safe-area-inset-bottom))] lg:pb-16">
      <motion.header
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}
        className="text-center px-4 sm:px-6 md:px-12 pt-8 sm:pt-10 md:pt-12 pb-7 sm:pb-9 border-b border-canvas/10"
      >
        <nav
          className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 font-mono text-[8px] uppercase tracking-[0.2em] text-taupe-muted mb-4 sm:mb-5"
          aria-label="Breadcrumb"
        >
          <Link to="/shop" className="hover:text-canvas transition-colors">
            Shop
          </Link>
        ...
        <h1 className="font-serif text-[clamp(1.75rem,5vw,2.75rem)] tracking-tight leading-tight">
          Shopping cart
        </h1>
        {cart.length > 0 && (
      ...
      <div className="px-4 sm:px-6 md:px-12 py-7 sm:py-10 w-full max-w-6xl mx-auto">
        {cart.length === 0 ? (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(280px,360px)] gap-8 lg:gap-12 xl:gap-14 items-start">
            <div className="min-w-0">
                  ...
                  >
                    <div className="flex gap-3 sm:gap-4 md:gap-5 min-w-0">
                      <button
                        ...
                        className="shrink-0 pt-1 font-mono text-base text-taupe-muted hover:text-canvas transition-colors cursor-pointer leading-none"
                      ...
                      <button
                        ...
                        className="w-[72px] sm:w-20 md:w-[88px] shrink-0 aspect-[4/5] flex items-end justify-center cursor-pointer"
                      >
                        <img
                          src={item.image}
              ...
              <Link
                to="/shop"
                className="inline-block mt-6 sm:mt-8 font-sans text-[9px] uppercase tracking-[0.22em] text-taupe-muted hover:text-canvas transition-colors"
              >
                ← Continue shopping
              </Link>
            </div>
            ...
            <motion.aside
              ...
              className="cart-summary lg:sticky lg:top-24 xl:top-28 pt-6 lg:pt-0 lg:border-t-0 border-t border-canvas/10 w-full"
            >
              ...
              <Link
                to="/checkout"
                className="checkout-btn w-full text-center mt-6 sm:mt-7 hidden lg:block"
              >
                Check out
              </Link>

              <p className="font-body-italic italic text-[11px] text-taupe-muted font-light mt-5 leading-relaxed text-center hidden lg:block">
      ...
      {cart.length > 0 && (
        <div className="cart-mobile-bar lg:hidden fixed inset-x-0 bottom-0 z-40 border-t border-canvas/10 bg-cream/95 backdrop-blur-md px-4 sm:px-6 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] flex items-center justify-between gap-4">
          <div className="min-w-0">
            <p className="font-mono text-[8px] uppercase tracking-[0.18em] text-taupe-muted">
              Total
            </p>
            <p className="font-serif text-lg tabular-nums text-canvas truncate">
              {formatNgn(totalWithShipping)}
            </p>
          </div>
          <Link to="/checkout" className="checkout-btn shrink-0 px-6 sm:px-8">
            Check out
          </Link>
        </div>
      )}
